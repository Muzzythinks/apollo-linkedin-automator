...
async function markTaskDone(apiKey, taskId, dryRun, emit) {
  if (dryRun) { emit({ type: 'log', message: `  [dry-run] Would mark task ${taskId} done` }); return; }
  const res = await apolloPost(apiKey, `/tasks/${taskId}/mark_as_done`);
  if (res.task?.completed_at) {
    emit({ type: 'log', message: '  Marked done in Apollo.' });
  } else {
    emit({ type: 'log', message: '  Warning: unexpected mark-done response' });
  }
}
 ...
 * @param {string} opts.profileDir  - persistent Chromium profile directory
 ...
 */
async function runTasks({ apiKey, profileDir, progressPath, dailyCountsPath, maxActions = 30,
  ...
  try {
    for (let i = 0; i < pending.length; i++) {
      ...
      emit({ type: 'task_start', index: taskIndex, total: pending.length, task });
      ...
      if (!linkedinUrl) {
        ...
        emit({ type: 'task_done', index: taskIndex, outcome: 'failed' });
        failed++;
        continue;
      }
      ...
      try {
        ...
        if (result === 'not_found') {
          notFound++;
          emit({ type: 'task_done', index: taskIndex, outcome: 'skipped' });
          ...
          try {
            await markTaskDone(apiKey, task.id, dryRun, emit);
          } catch (err) {
            emit({ type: 'log', message: `  Failed to mark done in Apollo: ${err.message}` });
          }
          sent++;
          emit({ type: 'task_done', index: taskIndex, outcome: result === 'already_connected' ? 'already_connected' : 'sent' });
        } else {
          failed++;
          emit({ type: 'task_done', index: taskIndex, outcome: 'failed' });
        }
...
module.exports = { runTasks, fetchLinkedInTasks, loadDailyCounts };
