...
// Fetch a single task's full detail via Apollo's internal web API. Needed for
// message tasks because /tasks/search omits the rendered message body —
// it's only on the per-task endpoint at task.linkedin_emailer_template.body_text
// (already rendered with contact fields filled in).
async function fetchTaskDetail(context, taskId) {
  ...
  const result = await apolloPage.evaluate(async id => {
    const getCsrf = () => {
      ...
      const m = document.cookie.match(/(?:^|; )(?:X-CSRF-TOKEN|XSRF-TOKEN|_csrf_token|csrf_token)=([^;]+)/i);
      if (m) return decodeURIComponent(m[1]);
      return null;
    };
    const csrf = getCsrf();
    const res = await fetch(`/api/v1/tasks/${id}?cacheKey=${Date.now()}`, {
      credentials: 'include',
      headers: {
        ...(csrf ? { 'X-CSRF-Token': csrf } : {}),
        'X-Referer-Host': 'app.apollo.io',
        'X-Referer-Path': `/tasks/${id}`,
      },
    });
    return { status: res.status, body: await res.text().catch(() => '') };
  }, taskId);
  if (opened) await apolloPage.close().catch(() => {});
  if (result.status >= 400) throw new Error(`GET /tasks/${taskId} ${result.status}: ${result.body.slice(0, 200)}`);
  try { return JSON.parse(result.body).task || {}; }
  catch { throw new Error(`GET /tasks/${taskId} non-JSON response`); }
}

// Update the draft message body stored on an Apollo task. Mirrors what the
// Apollo UI does when the user edits the textarea: PUT /api/v1/tasks/{id}
// with standalone_outreach_task_message.body_text. The runner later reads
// this back via fetchTaskDetail when it's time to send.
async function updateTaskMessage(context, taskId, bodyText) {
  let apolloPage = context.pages().find(p => p.url().includes('app.apollo.io'));
  let opened = false;
  if (!apolloPage) {
    apolloPage = await context.newPage();
  ...
  const result = await apolloPage.evaluate(async ({ id, bodyText }) => {
    const getCsrf = () => {
      const meta = document.querySelector('meta[name="csrf-token"]');
      if (meta?.content) return meta.content;
      const m = document.cookie.match(/(?:^|; )(?:X-CSRF-TOKEN|XSRF-TOKEN|_csrf_token|csrf_token)=([^;]+)/i);
    ...
    const res = await fetch(`/api/v1/tasks/${id}`, {
      ...
      body: JSON.stringify({
        standalone_outreach_task_message: { body_text: bodyText },
        include_linkedin_emailer_template: 'true',
        note: '',
        cacheKey: Date.now(),
      }),
    });
  ...
  if (opened) await apolloPage.close().catch(() => {});
  if (result.status >= 400) throw new Error(`PUT /tasks/${taskId} ${result.status}: ${result.body.slice(0, 200)}`);
}
...
// sends CSRF/origin headers that context.request.post doesn't replicate, so
// run the fetch from inside an Apollo page — the browser handles all the
// auth headers natively.
// Endpoint + body shape discovered via sniff-api.js:
//   POST /api/v1/tasks/{id}/complete
//   body: { note, user_id, cacheKey }
// (The bulk_execute_and_complete endpoint with execute:false does NOT actually
// set completed_at — it's a classifier used by the Apollo UI, not a completer.)
async function markTaskDone(context, task, dryRun, emit) {
  const taskId = typeof task === 'string' ? task : task.id;
  const userId = typeof task === 'object' ? task.user_id : null;
  if (dryRun) { emit({ type: 'log', message: `  [dry-run] Would mark task ${taskId} done` }); return; }
  if (!userId) throw new Error(`mark-done missing user_id for task ${taskId}`);
  ...
  const result = await apolloPage.evaluate(async ({ id, userId }) => {
    // Apollo's SPA requires an x-csrf-token (rotating) plus a couple of
    // custom referer headers. The token usually lives in a meta tag or a
    // cookie — try both.
    const getCsrf = () => {
      const meta = document.querySelector('meta[name="csrf-token"]');
      if (meta?.content) return meta.content;
    ...
    const res = await fetch(`/api/v1/tasks/${id}/complete`, {
      ...
      headers: {
        ...
        ...(csrf ? { 'X-CSRF-Token': csrf } : {}),
        'X-Referer-Host': 'app.apollo.io',
        'X-Referer-Path': location.pathname + location.hash,
        'X-Accept-Language': 'en',
      },
      body: JSON.stringify({ note: '', user_id: userId, cacheKey: Date.now() }),
    });
    return { status: res.status, body: await res.text().catch(() => ''), csrfFound: !!csrf };
  }, { id: taskId, userId });
  if (!result.csrfFound) {
    emit({ type: 'log', message: '  Warning: no CSRF token found on Apollo page — request may be rejected.' });
  }

  if (openedPage) await apolloPage.close().catch(() => {});

  if (result.status >= 400) {
    throw new Error(`/tasks/${taskId}/complete ${result.status}: ${result.body.slice(0, 200)}`);
  }
  let parsed = {};
  try { parsed = JSON.parse(result.body); } catch {}
  if (parsed.task?.completed_at) {
...
async function sendMessage(page, linkedinUrl, message, dryRun, emit) {
  ...
  if (!await msgBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
    // No Message button = not connected yet (or LinkedIn hiding it). Treat the
    // same way as a 404 — skip, mark done in Apollo so it doesn't re-appear.
    emit({ type: 'log', message: '  Message button not found — not connected yet, skipping.' });
    return 'not_found';
  }
...
async function runTasks({ apiKey, profileDir, context: providedContext, progressPath, dailyCountsPath, maxActions = 30,
  ...
  if (specificTasks) {
    ...
    // Over-fetch so 404s / already-pending skips don't eat into the action
    // budget. Apollo caps per_page at 100.
    const fetchSize = Math.min(100, Math.max(maxActions * 2, maxActions + 20));
    const { tasks: allTasks, total } = await fetchLinkedInTasks(apiKey, 1, fetchSize);
    emit({ type: 'log', message: `${total} total LinkedIn tasks pending. Target: ${maxActions} action${maxActions === 1 ? '' : 's'} (skips don't count).` });
  ...
  let failed = 0;
  let notFound = 0;
  let actionsTaken = 0; // only real connects/messages; 404s & pending don't count
  ...
  try {
    for (let i = 0; i < pending.length; i++) {
      ...
      // Stop when we've hit the action budget. Skipped/404 tasks processed so
      // far were free (no LinkedIn interaction, just a mark-done API call).
      if (!specificTasks && actionsTaken >= maxActions) {
        emit({ type: 'log', message: `Reached target of ${maxActions} action${maxActions === 1 ? '' : 's'}.` });
        break;
      }
      ...
      const budgetLabel = specificTasks ? `${taskIndex}` : `action ${actionsTaken + 1}/${maxActions}`;
      emit({ type: 'log', message: `\n[${budgetLabel}] ${task.type.replace('linkedin_step_', '').toUpperCase()} — ${contact.name || task.contact_id}` });
      ...
      try {
        if (task.type === 'linkedin_step_connect') {
          ...
          // Resolve the message body from Apollo (search API omits it).
          let messageBody = note;
          try {
            const detail = await fetchTaskDetail(context, task.id);
            messageBody = detail.linkedin_emailer_template?.body_text
              || detail.standalone_outreach_task_message?.body_text
              || detail.note
              || '';
          } catch (err) {
            emit({ type: 'log', message: `  Failed to load message body: ${err.message}` });
          }
          if (!messageBody) {
            emit({ type: 'log', message: '  No message body found on task, skipping.' });
            emit({ type: 'task_done', index: taskIndex, taskId: task.id, outcome: 'failed' });
            failed++;
            continue;
          }
          emit({ type: 'log', message: `  Message: "${messageBody.slice(0, 100).replace(/\n/g, ' ')}${messageBody.length > 100 ? '…' : ''}"` });
          result = await sendMessage(page, linkedinUrl, messageBody, dryRun, emit);
        } else {
        ...
        if (result === 'not_found') {
          // 404 profiles and already-pending connections can't be completed by
          // the bot, but leaving them in Apollo's queue means they re-appear on
          // every run. Mark them done too.
          if (!dryRun) {
            completed.add(task.id);
            saveProgress(progressPath, completed);
          }
          try {
            await markTaskDone(context, task, dryRun, emit);
          } catch (err) {
...
module.exports = { runTasks, fetchLinkedInTasks, fetchTaskDetail, updateTaskMessage, markTaskDone, loadProgress, saveProgress, loadDailyCounts, saveDailyCounts };
