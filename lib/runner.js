...
// Apollo's mark-complete is an internal endpoint on app.apollo.io authed via
// the session cookie (not the public API key). Route it through the browser
// context so the Apollo cookies captured on login get sent.
...
async function markTaskDone(context, taskId, dryRun, emit) {
  ...
  const res = await context.request.post(
    'https://app.apollo.io/api/v1/tasks/bulk_execute_and_complete',
    {
      data: {
        ids: [taskId],
        execute: false,
  ...
  if (!res.ok()) {
    ...
    throw new Error(`bulk_execute_and_complete ${res.status()}: ${body.slice(0, 200)}`);
  }
  let body = {};
  try { body = await res.json(); } catch {}
  const completed = Array.isArray(body.tasks) && body.tasks.some(t => t.id === taskId);
  if (completed) {
    emit({ type: 'log', message: '  Marked done in Apollo.' });
  } else {
    emit({ type: 'log', message: `  Marked done in Apollo (response: ${JSON.stringify(body).slice(0, 120)})` });
  }
}
...
async function runTasks({ apiKey, profileDir, context: providedContext, progressPath, dailyCountsPath, maxActions = 30,
  ...
  if (ownContext) {
    // Clean up stale Chromium profile locks left behind by a crashed prior run
