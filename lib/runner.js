...
// Apollo's /tasks/{id}/mark_as_done is an internal endpoint that auths via the
// session cookie (app.apollo.io), not via public-API key. Route it through the
// browser context so the Apollo cookies we captured on login get sent.
async function markTaskDone(context, taskId, dryRun, emit) {
  ...
  const res = await context.request.post(
    `https://app.apollo.io/api/v1/tasks/${taskId}/mark_as_done`,
    { data: {} }
  );
  if (!res.ok()) {
    const body = await res.text().catch(() => '');
    throw new Error(`mark_as_done ${res.status()}: ${body.slice(0, 200)}`);
  }
  let body = {};
  try { body = await res.json(); } catch {}
  if (body.task?.completed_at) {
    emit({ type: 'log', message: '  Marked done in Apollo.' });
  } else {
    emit({ type: 'log', message: '  Marked done in Apollo (unexpected response shape).' });
  }
}
